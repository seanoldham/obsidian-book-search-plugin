...
export class GoogleBooksApi implements BaseBooksApiImpl {
  ...
  private extractBasicBookInfo(item: VolumeInfo): Partial<Book> {
    return {
      ...
      coverUrl: this.setCoverImageEdgeCurl(item.imageLinks?.large ?? '', this.enableCoverImageEdgeCurl),
      coverSmallUrl: this.setCoverImageEdgeCurl(item.imageLinks?.thumbnail ?? '', this.enableCoverImageEdgeCurl),
